...
exports.getTasks = async (req, res) => {
  try {
    ...
    if (!sort_priority) {
      tasks.sort((a, b) => {
        // Move completed tasks to the end irrespective of priority
        if (a.task_completed && !b.task_completed) return 1;
        if (!a.task_completed && b.task_completed) return -1;
        return 0;
      });
    } else {
      // If sort_priority query parameter is provided, sort tasks based on priority
      tasks.sort((a, b) => {
        const priorityOrder = { high: 1, medium: 2, low: 3 };

        if (sort_priority === 'false') {
          // Sort in descending order (high to low)
          return priorityOrder[b.task_priority] - priorityOrder[a.task_priority];
...
exports.postTask = async (req, res) => {
  ...
  try {
    ...
    const task = await Task.create({ user: req.user.id, description, task_priority, task_completed });
...
exports.putTask = async (req, res) => {
  ...
  try {
    ...
    task = await Task.findByIdAndUpdate(req.params.taskId, { description, task_priority, task_completed }, { new: true });
